...
if is_step_complete "$CURRENT_STEP"; then
    ...
    if ! confirm "Reinstall Nix?"; then
        ...
        info "Installing Nix..."
        if retry_command curl --proto '=https' --tlsv1.2 -sSf -L https://install.determinate.systems/nix | sh -s -- install; then
    ...
    if retry_command bash -c "curl --proto '=https' --tlsv1.2 -sSf -L https://install.determinate.systems/nix | sh -s -- install"; then
...
Recovery steps:
  ...
  2. Visit https://determinate.systems/posts/determinate-nix-installer
